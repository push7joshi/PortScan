...
void Scan::my_callback(u_char* scanObj, const struct pcap_pkthdr* pkthdr,
	...
	if (find(knownService.begin(), knownService.end(), ntohs(mySelf->port))
			!= knownService.end()) {
		serviceName = servChk(mySelf->ipToScan, ntohs(mySelf->port));
	} else {
		struct servent * service = getservbyport(mySelf->port,
				protoEntry->p_name);
		if (service != NULL)
		...
		if (sendto(socketFD, buffer, sizeof(struct ip) + sizeof(struct tcphdr),
			...
			if (dispatch_msg == -1) {
				...
				if (retries > 0) {
					...
					cout
							<< "unable to get connection after all the retries allowed\n";
					return;
				}
			}
		}
